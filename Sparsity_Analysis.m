...
[Mdl,FitInfo,HyperparameterOptimizationResults] = fitclinear(noninv_datmat', logic_outcm','ObservationsIn','columns',...
    'Learner', 'svm', ...
    'Regularization', 'lasso', 'batchSize', 16, 'OptimizeLearnRate', true, ...
    'OptimizeHyperparameters', {'Lambda'}, 'HyperparameterOptimizationOptions' ,...
    struct('MaxObjectiveEvaluation', 60, 'repartition',true, 'KFold', 10, 'verbose', 1));

noninv_non0ind = find(Mdl.Beta);
noninv_coefs = Mdl.Beta(Mdl.Beta~=0);
%% For invasive
inv_datmat = dat_mat(:, 8: end-4);


[Mdl_inv,FitInfo_inv,HyperparameterOptimizationResults_inv] = fitclinear(inv_datmat', logic_outcm',...
    'ObservationsIn','columns',...
    'Learner', 'svm', ...
    'Regularization', 'lasso', 'batchSize', 16, 'OptimizeLearnRate', true, ...
    'OptimizeHyperparameters', {'Lambda'}, 'HyperparameterOptimizationOptions' ,...
    struct('MaxObjectiveEvaluation', 60, 'repartition',true, 'KFold', 10, 'verbose', 1) );
%Obtain search lambda range from these e.g., Lambda = logspace(-6,-0.5,11);
% for each lambda train a invasive-non-invasive  compute auc
